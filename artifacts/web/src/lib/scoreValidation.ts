...
export const SPEAKER_RANGE_MESSAGE = `درجة المتحدث يجب أن تكون رقماً صحيحاً بين ${SPEAKER_MIN} و ${SPEAKER_MAX}`;
export const REPLY_RANGE_MESSAGE = `درجة خطاب الرد يجب أن تكون رقماً صحيحاً بين ${REPLY_MIN} و ${REPLY_MAX}`;
...
export function clampScoreInput(
  ...
  if (raw.trim() === "") return "";
  // Whole numbers only — decimal points and separators are dropped.
